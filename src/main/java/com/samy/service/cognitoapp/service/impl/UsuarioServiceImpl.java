...
import static com.samy.service.cognitoapp.utils.Utils.convertActualZoneLocalDateTime;
...
public class UsuarioServiceImpl extends CrudImpl<Usuario, String> implements UsuarioService {
  ...
  public void registrarColaboratorDesdeSami(ColaboratorRequest request) {
    ...
    List<ColaboradorTable> colaboradores =
        colaboradorService.buscarPorIdUsuario(usuario.getIdUsuario());
    Long countColaboradores =
        colaboradores.stream().filter(item -> item.getCorreo().equals(request.getMail())).count();
    ...
    if (countColaboradores != 0 && existeColaborador) {
      throw new BadRequestException(
          "Ya existe un colaborador con el mismo UserName : " + request.getUserName());
    }
    ...
    ColaboradorTable colaboradorSave = ColaboradorTable.builder().nombres(request.getNames())
        .apellidos(request.getLastName()).correo(request.getMail())
        .password(password.generarPassword()).empresa(usuario.getEmpresa())
        .idUsuario(usuario.getIdUsuario()).fechaRegistro(LocalDateTime.now()).passwordChanged(false)
        .accesos(new ArrayList<>()).estado(true).validado(false).eliminado(false).build();
    ...
    dynamicTemplate.addProperty("token",
  ...
  public UserResponseBody getUsuarioByUserName(String userName) {
    ...
    return UserResponseBody.builder().id(usuario.getIdUsuario())
        .datosUsuario(nuevoNombre.concat(" ").concat(nuevoApellido))
        .nombreUsuario(usuario.getNombreUsuario()).empresa(usuario.getEmpresa()).claveCambiada(true)
        .rol(usuario.getRol()).accesos(usuario.getAccesos().stream()
  ...
  public Usuario registrarUsuarioV2(UserRequestBody requestBody) {
     ...
     * extraerNombreUsuario(requestBody.getNombreUsuario())); objMailBody.addProperty("nombres",
     * requestBody.getNombres()); objMailBody.addProperty("apellidos", requestBody.getApellidos());
    ...
    if (fechaExp.isAfter(LocalDateTime.now())) {
      ...
      if (usuario.isValidado()) {
        ...
        UserResponseBody response = cognitoService.registrarUsuarioV2(UserRequestBody.builder()
            .nombres(usuario.getNombres()).apellidos(usuario.getApellidos())
  ...
  public boolean activarColaborador(String tokenKey) {
    ...
    if (fechaExp.isAfter(LocalDateTime.now())) {
      log.info("fechaExp : " + fechaExp);
      ColaboradorTable colaboradorTable =
          colaboradorService.buscarPorId(idColaborador.replace("\"", ""));
  ...
  public UsuariosPorEmpresaResponse verPorEmpresa(String empresa) {
    List<Usuario> usuariosPorEmpresa =
        repo.findByEmpresa(empresa).stream().collect(Collectors.toList());
    LocalDateTime fechaRegistro = LocalDateTime.now();
    if (!usuariosPorEmpresa.isEmpty()) {
      fechaRegistro = usuariosPorEmpresa.get(0).getFechaCreacion();
    }
    List<String> usuarios =
        usuariosPorEmpresa.parallelStream().map(Usuario::getCorreo).collect(Collectors.toList());
    List<String> colaboradores = usuariosPorEmpresa.parallelStream()
        .sorted(Comparator.comparing(Usuario::getNombreUsuario)).flatMap(item -> {
          return colaboradorService.buscarPorIdUsuario(item.getIdUsuario()).stream()
              .map(colaborador -> {
                return colaborador.getCorreo();
              });
        }).distinct().collect(Collectors.toList());
    usuarios.addAll(colaboradores);
    return UsuariosPorEmpresaResponse
        .builder()
        .empresa(empresa)
        .fechaCreacion(formatoFecha(convertActualZoneLocalDateTime(fechaRegistro)))
        .usuarios(usuarios.stream().sorted().collect(Collectors.toList())).build();
  }
}
